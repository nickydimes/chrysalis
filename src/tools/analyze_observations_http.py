...
async def main():
    """
    Connects to the gemini-mcp HTTP server via StreamableHTTP transport,
    reads processed JSON notes, and analyzes each using the
    observation_analysis prompt template.
    """
    ...
    try:
        ...
        try:
            ...
            async with streamable_http_client(url=server_url, headers=headers) as (
                read,
                write,
            ):
                async with ClientSession(read, write) as session:
                    ...
                    print(
                        f"--- Connection established. Analyzing {len(json_files)} notes... ---"
                    )
                    ...
                    for filename in json_files:
                        ...
                        prompt = template.format(
                            metadata=metadata_str, protocol_steps=steps_str
                        )

                        source = note_data.get("metadata", {}).get(
                            "original_file", filename
                        )
                        print(f"\n--- Analyzing: {source} ---")

                        analysis_result = await session.call_tool(
                            "gemini_chat", {"message": prompt}
                        )
            ...
            print(f"Error connecting or initializing MCP session: {conn_e}")
            raise
